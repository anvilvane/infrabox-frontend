...
const ALLOWED_PATCH_FIELDS = ['status', 'notes', 'assigned_slug'];
...
function forwardError(e) {
  ...
  console.error('admin partner-onboarding item error', e);
...
export async function GET(_req, { params }) {
  ...
  try {
    const data = await callInternalApi(
      'GET',
      `/v1/internal/partner-directory/onboarding/${encodeURIComponent(id)}`
    );
